...
const deleteListing = () => {
  if (confirm("Are you sure?", props.listing.id)) {
...
<template>
  ...
  <!-- Admin -->
  <div
    v-if="$page.props.auth.user.role && $page.props.auth.user.role === 'admin'"
      ...
      <div v-if="listing.tags" class="mb-6">
        ...
        <div class="flex items-center gap-3">
          <div v-for="tag in listing.tags.split(',')" :key="tag">
            <Link
              :href="route('home', { tag })"
              class="bg-slate-500 text-white py-px px-2 rounded-full hover:bg-slate-700 dark:hover:bg-slate-900"
            >
              {{ tag }}
            </Link>
          </div>
